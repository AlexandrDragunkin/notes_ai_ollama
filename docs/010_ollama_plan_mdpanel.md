...
Тесты находятся в `src/Proto/mDPanel/tests/` и запускаются через `pytest`. CI (GitHub Actions) включает линтеры и проверку типизации (mypy).
...
* Сложные геометрические расчёты – использовать `shapely` (добавить в `requirements.txt`).  
* Регрессия старого кода – обязательный запуск полного набора тестов при каждом коммите.
